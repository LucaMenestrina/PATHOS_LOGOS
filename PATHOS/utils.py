...
def download(
    ...
    if retrieved_content is not None:
        ref_date = retrieved_date if retrieved_date else datetime.now().astimezone(timezone.utc)
        filename = retrieved_filename if retrieved_filename else str(ref_date).replace(" ", "_")
    ...
    if update or (update == None and ref_date > file_date):
        ...
        if retrieved_content is not None:
            try:
                retrieved_content.to_csv(filepath, sep="\t", index=False)
            except:
                with open(filepath, "wb") as output:
                    output.write(retrieved_content)
        elif "http" in url:
            if not post:
...
class Database:
    ...
    def _add_file(
        ...
        tmp_file = File(
            ...
            post=post,
            data_to_send=data_to_send,
            sequential_identifier=sequential_identifier if sequential_identifier != None else len(self.__files),
            comment=comment,
            index_col=index_col,
            retrieved_content=retrieved_content,
            retrieved_filename=retrieved_filename,
...
def get_best_match(query, choices, score_cutoff=90):
...
class SQLdatabase():
    """
    Utility class for interacting with the postgres database
    """
    ...
    def _open_connection(self):
        ...
        """
        if not hasattr(self, f"_{self.__class__.__name__}__connection"):
            ...
            log.warning("Connection was already open")
    ...
    def _close_connection(self):
        ...
        """
        if not hasattr(self, f"_{self.__class__.__name__}__connection"):
            log.error("Connection has never been opened")
            raise
        elif self.__connection.closed:
    ...
    def query(self, statement, formatted=True):
        """
        Open connection, query the database, close connection
        """
        # initial checks
        if not isinstance(statement, str):
            log.error("'statement' should be a string")
            raise
        ...
        if formatted:
            if len(fetched):
                # forat query output in pandas DataFrame
                output = pd.DataFrame(fetched, columns=columns)
            else:
                output = pd.DataFrame()
